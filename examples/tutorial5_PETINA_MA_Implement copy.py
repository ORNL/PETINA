...
# def applyDPLaplace(tensor, epsilon_per_param, accountant=None, sensitivity=1.0):
#     """
...
#         sensitivity (float): The L1 sensitivity of the query/function (usually clipping norm).
    
#     Returns:
#         torch.Tensor or np.ndarray: The tensor with added Laplace noise.
#     """
#     # if accountant is not None:
#     #     print("Using accountant")
#     # else:
#     #     print("No accountant provided")
#     scale = sensitivity / epsilon_per_param
#     noise = np.random.laplace(loc=0, scale=scale, size=tensor.shape)
    
#     # Calculate privacy cost and spend it per parameter
#     if accountant is not None:
#         print("Accountant is present, spending budget for Laplace noise addition.")
#         # For Laplace, delta is 0
#         cost_epsilon, cost_delta = epsilon_per_param, 0.0
...
#     return tensor + noise
def applyDPLaplace(domain, sensitivity=1, epsilon=1, gamma=1, accountant=None):
    """
    ...
    if accountant is not None:
        ...
        print("Total spent budget so far: %r" % (accountant.spent(),))
