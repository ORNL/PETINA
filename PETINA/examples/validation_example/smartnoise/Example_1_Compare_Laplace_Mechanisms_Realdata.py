# """
# Example_1_Compare_Laplace_Mechanisms_Realdata

# Compare Laplace mechanism outputs between OpenDP (SmartNoise) and PETINA
# using real-world datasets: MNIST, CIFAR-10, and Iris.

# Includes plots of noisy values distribution saved to disk.
# """
...
# # Utility Functions
# # ----------------------------
# def run_trials(true_value, sensitivity, label):
#     """
#     Run Laplace noise trials with OpenDP and PETINA.
#     true_value: scalar float to privatize
...
#     # Prepare input vector for OpenDP as list of floats (1D vector)
#     data = [float(true_value)]

#     for _ in trange(num_trials, desc=f"{label}: Running trials"):
#         # OpenDP requires list of floats
#         noisy_opendp = laplace_mech(data)
#         noisy_opendp_results.append(noisy_opendp[0])  # unpack single float
...
#         )
#         noisy_petina_results.append(noisy_petina[0])  # unpack single float

#     return np.array(noisy_opendp_results), np.array(noisy_petina_results)

# def summarize(results, true_value, label):
#     """
#     Print mean, std, and bias of noisy results.
#     """
#     mean = results.mean()
#     std = results.std()
#     bias = mean - true_value

#     print(f"\n{label} Laplace Noise Statistics:")
#     print(f"  Mean = {mean:.4f}, Std = {std:.4f}, Bias = {bias:.4f}")
#     print(f"Total Trials = {len(results)}")

# def plot_distributions(opendp_results, petina_results, true_value, label):
#     """
#     Plot histogram distributions for OpenDP and PETINA results.
#     Saves output to Plot/ directory.
#     """
...
#     filename = os.path.join("Plot", f"Example_1_{label.lower().replace(' ', '_')}_laplace_distribution.png")
...
# sensitivity_mnist = 1.0 / 100  # Mean of 100 samples, pixel values in [0,1]

# opendp_mnist, petina_mnist = run_trials(mnist_avg, sensitivity_mnist, "MNIST")
# summarize(opendp_mnist, mnist_avg, "OpenDP (MNIST)")
# summarize(petina_mnist, mnist_avg, "PETINA (MNIST)")
# plot_distributions(opendp_mnist, petina_mnist, mnist_avg, "MNIST")
...
# sensitivity_cifar = 1.0 / 100  # Mean of 100 samples, pixel values in [0,1]

# opendp_cifar, petina_cifar = run_trials(cifar_avg, sensitivity_cifar, "CIFAR-10")
# summarize(opendp_cifar, cifar_avg, "OpenDP (CIFAR-10)")
# summarize(petina_cifar, cifar_avg, "PETINA (CIFAR-10)")
# plot_distributions(opendp_cifar, petina_cifar, cifar_avg, "CIFAR-10")

# # ----------------------------
# # 3. Iris Dataset
# # ----------------------------
# iris = load_iris()
# petal_lengths = iris.data[:, 2]  # Petal length column
# iris_sample = np.random.choice(petal_lengths, size=30, replace=False)
# iris_avg = np.mean(iris_sample)
# sensitivity_iris = (iris_sample.max() - iris_sample.min()) / 30  # Conservative bound for mean

# opendp_iris, petina_iris = run_trials(iris_avg, sensitivity_iris, "Iris")
# summarize(opendp_iris, iris_avg, "OpenDP (Iris)")
# summarize(petina_iris, iris_avg, "PETINA (Iris)")
# plot_distributions(opendp_iris, petina_iris, iris_avg, "Iris")
...
num_trials = 5000  # Reduce trials for speed
datasets_info = {}
...
def plot_histogram(opendp_results, petina_results, true_value, label, epsilon):
    ...
    plt.grid(True)
    os.makedirs("Plot", exist_ok=True)
    filename = os.path.join("Plot", f"{label.lower().replace(' ', '_')}_epsilon_{epsilon}_histogram.png")
    plt.savefig(filename)
    plt.close()
